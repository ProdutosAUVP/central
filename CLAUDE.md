...
**Catálogo de menus (Design System)** — `src/components/widgets/MenusSuperiores.tsx` e `src/components/widgets/MenusLaterais.tsx` reúnem seis modelos de menu superior e seis de menu lateral, cada um num `ComponentShowcase` próprio, nas seções `/design-system#menus-superiores` e `#menus-laterais`. São modelos **independentes** (institucional, mega menu de catálogo, barra flutuante, portal em duas linhas, pílula deslizante, busca protagonista; documentação, índice sem fundo, trilho de ícones, workspace recolhível, drawer sobreposto, árvore de referência) — existem para a liderança escolher um ou dois, não para serem variações de um mesmo componente.

Os doze usam **a mesma** navegação de exemplo, que vive em `src/data/menusCatalogo.ts` (itens, filhos, grupos, utilitários e produtos). É o ponto do catálogo: com os mesmos destinos em todo lugar, a diferença que sobra na tela é só a decisão de design. Mexer num rótulo lá muda os doze de uma vez. A exceção é o portal em duas linhas, onde cada produto recorta essa mesma lista — trocar de produto troca a cor da faixa e os itens disponíveis.
