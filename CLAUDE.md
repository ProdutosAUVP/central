...
**Catálogo de menus (Design System)** — `src/components/widgets/MenusSuperiores.tsx` e `src/components/widgets/MenusLaterais.tsx` reúnem seis modelos de menu superior e seis de menu lateral, cada um num `ComponentShowcase` próprio, nas seções `/design-system#menus-superiores` e `#menus-laterais`. São modelos **independentes** (institucional, mega menu de catálogo, barra flutuante, portal em duas linhas, pílula deslizante, busca protagonista; documentação, editorial escura, trilho de ícones, workspace recolhível, drawer sobreposto, árvore de referência) — existem para a liderança escolher um ou dois, não para serem variações de um mesmo componente. Os doze modelos são navegáveis de verdade — clicar troca a "página" mostrada na moldura, os painéis fecham no Esc, no clique fora e ao sair com o mouse, e o item ativo leva `aria-current`. Todo popover vive **dentro** da moldura do modelo, que reserva altura para ele: o `ComponentShowcase` tem `overflow-hidden` na raiz e cortaria qualquer overlay que vazasse. Os modelos escuros por decisão de estilo (barra flutuante e editorial) usam cores literais (`bg-[#09090b]`, `text-[#a1a1aa]`) em vez de `bg-zinc-950`/`text-zinc-400`: as travas de contraste de `.dark` no `src/index.css` neutralizam as classes hardcoded da paleta Tailwind e apagariam a superfície escura no tema escuro.
