...
export function CodeBlock({ code, language = "tsx", className, tabs, collapsible = false, collapsibleLabel = "Ver código" }: CodeBlockProps) {
    ...
    <div className={cn("relative rounded-lg border bg-muted/50 overflow-hidden", !collapsible && className)}>
      <div className="flex items-center justify-between px-4 py-2 border-b bg-muted/80">
        <div className="flex items-center gap-1">
          {tabs ? (
            tabs.map((tab, i) => (
              <button key={tab.label} onClick={() => setActiveTab(i)} className={cn("px-3 py-1 text-xs font-bold uppercase tracking-wider rounded transition-colors", activeTab === i ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground")}>{tab.label}</button>
            ))
          ) : (
  ...
  return (
    <div className={cn("border rounded-lg overflow-hidden bg-card", className)}>
      <button onClick={() => setOpen((o) => !o)} className="w-full flex items-center justify-between px-6 py-3 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted/30 transition-colors">
