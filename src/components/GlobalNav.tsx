...
const systems = [
  {
    id: "hub",
    label: "Central AUVP",
    description: "Hub interno — acessos rápidos, novidades e produtos",
    icon: Home,
    path: "/",
  },
  {
    ...
    description: "Conheça o time, pilares, conexões e processo de trabalho",
    icon: Users,
    path: "/time",
  },
  {
...
export function GlobalNav() {
  ...
  return (
    <div className="relative flex items-center gap-3">
      {/* Logo */}
      <div className={cn("flex items-center justify-center h-8 w-8 md:h-10 md:w-10 rounded-lg", brand === "capital" ? "bg-brand-dark" : "bg-brand")}>
        <img src={brand === "capital" ? olhoBranco.url : olhoPreto.url} alt="AUVP Logo" className="h-5 w-5 md:h-7 md:w-7" />
      </div>
      ...
      <DropdownMenu onOpenChange={(open) => { if (open) dismissWelcome(); }}>
        <DropdownMenuTrigger className="flex items-center gap-2 outline-none group rounded-xl px-2 py-1.5 -mx-2 hover:bg-muted transition-colors">
          <div className="text-left">
            <p className="text-sm font-bold font-anek leading-tight text-foreground">{currentSystem.label}</p>
        ...
        <DropdownMenuContent align="start" className="w-80 p-2 space-y-1 bg-popover border border-border shadow-lg">
          ...
          {systems.map((system) => {
            ...
            return (
              ...
              >
                <div className={cn("flex items-center justify-center h-9 w-9 rounded-lg shrink-0 mt-0.5 border transition-colors", isActive ? "bg-foreground text-background border-foreground" : "bg-card text-foreground border-border")}>
                  <Icon className="h-4 w-4" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold font-anek text-foreground leading-tight">{system.label}</p>
                  <p className="text-xs text-muted-foreground leading-snug mt-1 font-roboto">{system.description}</p>
                </div>
                {isActive && (
                  <span className="text-[9px] font-bold text-foreground uppercase tracking-wider mt-1 px-1.5 py-0.5 rounded-md bg-background border border-border font-roboto shrink-0">Atual</span>
                )}
              </DropdownMenuItem>
            );
          })}
      ...
      {showWelcome && (
        <div className="absolute top-full left-12 mt-3 z-[60] animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="relative bg-popover border border-border rounded-xl p-4 shadow-xl w-[380px] max-w-[calc(100vw-2rem)] backdrop-blur-xl">
            ...
            <button onClick={dismissWelcome} className="absolute top-2 right-2 h-6 w-6 flex items-center justify-center rounded-md text-muted-foreground hover:text-foreground hover:bg-muted transition-colors" aria-label="Fechar">
              <X className="h-3.5 w-3.5" />
            </button>
            <div className="pr-6">
              <p className="text-[10px] font-bold font-roboto uppercase tracking-wider text-muted-foreground mb-1.5">Dica de navegação</p>
              <p className="text-sm font-anek font-bold text-foreground leading-tight mb-1">Navegue entre os sistemas</p>
              <p className="text-xs text-muted-foreground leading-relaxed font-roboto">Use este menu para acessar a Central, o Time de Produto, o Design System e o Manual de Tom e Voz da AUVP.</p>
            </div>
          </div>
        </div>
      )}
