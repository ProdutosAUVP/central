...
function ChoiceboxGroup({
  ...
  return (
    <div className="font-roboto">
      {showLabel && <div className="text-xs text-muted-foreground mb-2">{label}</div>}
      <div
        className={`flex gap-3 ${direction === "column" ? "flex-col" : "flex-row flex-wrap"}`}
          ...
          return (
            <button
              ...
              className={`flex items-start gap-3 text-left p-3 border transition-all min-w-[200px] flex-1 ${
                selected ? "border-primary bg-primary/5" : "border-border bg-background hover:border-foreground/30"
              } ${itemDisabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
              style={{ borderRadius: 12 }}
            >
              <span className="mt-0.5">
                <Indicator type={type} selected={selected} disabled={itemDisabled} />
              </span>
...
export function ChoiceboxRadio() {
  ...
  return (
    <ChoiceboxGroup label="Selecione um plano" type="radio" direction="row" items={PLAN_ITEMS} value={value} onChange={setValue} />
  );
}
...
export function ChoiceboxCheckbox() {
  ...
  return (
    <ChoiceboxGroup label="Selecione um plano" type="checkbox" direction="row" items={PLAN_ITEMS} value={value} onChange={setValue} />
  );
}
...
export function ChoiceboxDisabled() {
  ...
  return (
    <div className="flex flex-col items-stretch justify-start gap-6 flex-initial">
      <ChoiceboxGroup label="Grupo desabilitado" showLabel type="radio" direction="row" disabled items={PLAN_ITEMS} value={value} onChange={setValue} />
      <ChoiceboxGroup label="Item individual desabilitado" showLabel type="checkbox" direction="row" items={[{ ...PLAN_ITEMS[0], disabled: true }, PLAN_ITEMS[1]]} value={value2} onChange={setValue2} />
    </div>
  );
}
